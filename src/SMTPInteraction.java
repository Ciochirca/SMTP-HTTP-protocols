...
public class SMTPInteraction {
	...
	/* SMTP handshake. We need the name of the local machine.
	   Send the appropriate SMTP handshake command. */
        ...
        sendCommand("HELO friend!" + CRLF, 250);
	...
	/* Send all the necessary commands to send a message. Call
	   ...
	   exception thrown from sendCommand(). */
	    String mailFrom = "MAIL FROM: " + mailmessage.Sender + CRLF;
	    String rcptto = "RCPT TO: " + mailmessage.Recipient + CRLF;
	    String dataPlusHeaders = "DATA"+CRLF+mailmessage.Headers+CRLF;
	    String textMessage = mailmessage.Body+CRLF+"."+CRLF;

	    sendCommand(mailFrom, 250);
	    sendCommand(rcptto, 250);
	    sendCommand(dataPlusHeaders, 354);
	    sendCommand(textMessage, 250);
    }
	...
	/* Check that the server's reply code is the same as the parameter
	   rc. If not, throw an IOException. */
        ...
        if (!response.startsWith(String.valueOf(rc))) {
            throw new IOException("Command: " + command + "error, " + String.valueOf(rc) + " error");
        }
    }
